...
def insert_png_into_excel(excel_bytes, png_bytes, sheet_name="IMAGEN", cell="B5", scale=0.35):
    ...
    # Configurar la hoja para que LibreOffice genere exactamente 1 página PDF.
    # Sin esto, una imagen landscape en una hoja retrato desborda y genera páginas en blanco.
...
def safe_cell_write(ws, row, col, value, align='center'):
    """Escribe en una celda por row/col manejando celdas combinadas."""
    cell_obj = ws.cell(row=row, column=col)
    if isinstance(cell_obj, MergedCell):
        for merged_range in ws.merged_cells.ranges:
            if (merged_range.min_row <= row <= merged_range.max_row and
                    merged_range.min_col <= col <= merged_range.max_col):
                cell_obj = ws.cell(row=merged_range.min_row, column=merged_range.min_col)
                break
    ...
    if align:
        ...
        cell_obj.alignment = align_map.get(align, Alignment(horizontal='center'))
